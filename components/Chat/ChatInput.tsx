...
export const ChatInput = ({
  ...
  const handleSend = () => {
    ...
    onSend({ role: 'user', content: textInputContent }, plugin);
    setTextInputContent('');
    setPlugin(null);
  ...
  const handlePromptSelect = (prompt: Prompt) => {
    ...
    if (parsedVariables.length > 0) {
      console.log("setting promptModalVisible", true);
      homeDispatch({
        field: "promptModalVisible",
        value: true,
      });
    } else {
      const updatedContent = textInputContent?.replace(/\/\w*$/, prompt.content);
      setTextInputContent(updatedContent);
  ...
  const handleSubmit = (updatedVariables: string[]) => {
    const newContent = textInputContent?.replace(/{{(.*?)}}/g, (match, variable) => {
      const index = promptVariables.indexOf(variable);
  ...
  return (
    <div className="absolute bottom-0 left-0 w-full border-transparent bg-gradient-to-b from-transparent via-white to-white pt-6 dark:border-white/20 dark:via-[#343541] dark:to-[#343541] md:pt-2">
      <div className="stretch mx-2 mt-4 flex flex-row gap-3 last:mb-2 md:mx-4 md:mt-[52px] md:last:mb-6 lg:mx-auto lg:max-w-3xl">
        ...
        <div className="relative mx-2 flex w-full flex-grow flex-col rounded-md border border-black/10 bg-white shadow-[0_0_10px_rgba(0,0,0,0.10)] dark:border-gray-900/50 dark:bg-[#40414F] dark:text-white dark:shadow-[0_0_15px_rgba(0,0,0,0.10)] sm:mx-4">
          ...
          <textarea
            ...
            placeholder={
              t('Type a message or type / to select a template...') || ''
            }
      ...
      <div className="hidden sm:flex items-center justify-center px-3 pt-2 pb-3 text-center text-[12px] text-black/50 dark:text-white/50 md:px-4 md:pt-3 md:pb-6">
        <a 
          href="https://caduc.eus"
          target="_blank"
          rel="noreferrer" className="flex flex-nowrap items-center justify-center mr-2"
        >
        ...
        <span className="">
        {t(
          "Hold down spacebar to dictate audio for text-to-speech.",
        )}
        </span>
      </div>
    </div>
